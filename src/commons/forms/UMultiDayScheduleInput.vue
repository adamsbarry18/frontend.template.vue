...
<script setup lang="ts">
  ...
  const parsedTimeValue = computed(() => {
    if (input.value && input.value.split(' ').length === 6) {
      const value = `${input.value.split(' ')[2]}:${input.value.split(' ')[1]}`;
      lastTimeValue.value = value;
      return value;
    }
  ...
  const formattedValue = computed(() => {
    ...
    const formattedDays = dayValue.value
      .sort((a, b) => {
    ...
    const formattedMinute = timeValue.value
      ...
      : '00';
    return `00 ${formattedMinute} ${formattedHour} * * ${formattedDays}`;
  });
