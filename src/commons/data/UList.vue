...
<script setup lang="ts">
  ...
  const props = defineProps({
    ...
    defaultSort: {
      type: Object as PropType<{ prop: string | null; order: string | null }>,
    ...
    entityLabelKey: {
      type: String,
      default: i18n.global.t('commons.list-entities-count'),
    },
    entitySelectedLabelKey: {
      type: String,
      default: i18n.global.t('commons.list-selected-entities-count'),
    },
    entityIcon: {
      type: String,
      default: 'icon-object',
  ...
  const emit = defineEmits<{
    (e: 'filter-change', filter: Record<string, any>): void;
    (e: 'search-change', search: string): void;
    (e: 'selection-change', selection: any[]): void;
    (e: 'row-click', row: any): void;
    (e: 'row-rightclick', row: any, event: MouseEvent): void;
    (e: 'row-dblclick', row: any): void;
    (e: 'sort-change', event: { prop: string; order: string }): void;
  ...
  const filterChangeDebouncer = ref<ReturnType<typeof debounce> | null>(null);
  const searchChangeDebouncer = ref<ReturnType<typeof debounce> | null>(null);
  const onChangeDebouncer = ref<ReturnType<typeof debounce> | null>(null);
  ...
  function onCellHover(row: any, column: any, cell: HTMLElement) {
    ...
    if (!isScrolling.value) showRowButtons(row, cell.parentNode as HTMLElement);
  }
  ...
  function onRowClick(row: any, column: any, event: MouseEvent) {
    let rowNode = event.target as HTMLElement;
    while (rowNode && !rowNode.classList.contains('u-list-row'))
      rowNode = rowNode.parentNode as HTMLElement;
  ...
  function onSortChange(event: { prop: string; order: string }) {
    ...
    setDefaultPage(1);
    columnSortedComponent.value = (
      defineSlots()
        .default?.()
        ?.find(
          (c) => c.type.name === 'u-list-column' && c.props?.sortProp === prop
        ) as any
    )?.ref;
    onChange();
  }
  ...
  function parseTableColumns() {
    if (!props.backendSortable || !defineSlots().default) return;
    defineSlots()
      .default?.()
      .forEach((c) => {
