...
export function useNumbers() {
  ...
  const MAX_INT: number = 2147483647; // 2147483647 = 2^31 - 1
  /** Minimum 32-bit signed integer value */
  const MIN_INT: number = -MAX_INT - 1; // -2147483648 = -2^31
   ...
   */
  const signNumber = (value: number): string => (value >= 0 ? '+' : '');
   ...
   */
  function getCondenseThresholds(): CondenseThreshold[] {
    return [
   ...
   */
  function setPrecision(value: number, precision: number = 0): number {
   ...
   */
  function checkIsReducedNumberZero(
    value: number | string,
    maxPrecision: number
  ): boolean {
  ...
  return {
    ...
    MIN_INT,
    signNumber,
    getCondenseThresholds,
    setPrecision,
    checkIsReducedNumberZero,
    currencySymbols,
    numberFormat,
    ordinalize,
    formatFileSize,
